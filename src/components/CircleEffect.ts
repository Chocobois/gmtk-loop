...
export class CircleEffect extends Effect{
    ...
    private a: number = 0;
    constructor(scene: BaseScene, x: number, y: number, m: Monster, rad: number = 800, time: number = 1000, amount: number = 3) {
        super(scene,x,y);
        this.owner = m;
    ...
    redraw(){
        ...
        this.gfx.arc(0,0,this.curRad,0,360,false,0.001);
        this.gfx.closePath();
        this.gfx.strokePath();
        ...
        for(let i = 1; i < this.amt; i++){
            ...
            this.gfx.beginPath();
            //this.gfx.lineBetween(0,0,1920,1080);
            this.gfx.arc(0,0,((this.amt-i)/this.amt)*this.curRad,0,360,false,0.001);
            this.gfx.closePath();
            this.gfx.strokePath();
        }
